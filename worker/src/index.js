...
const unitOut = i => ({ id: UNITS[i].id, name: UNITS[i].name, cap: UNITS[i].cap, full: UNITS[i].name });
  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if (req.method !== 'POST' || !url.pathname.startsWith('/api/')) {
        if (url.pathname === '/') return new Response('haengseonji-world', { headers: { 'Content-Type': 'text/plain; charset=utf-8' } });
      ...
      if (url.pathname === '/api/free/guess' || url.pathname === '/api/free/giveup') {
        ...
        if (fg == null) return json({ error: '없는 나라' }, 400, h);
      ...
      if (url.pathname === '/api/guess') {
        ...
        if (g == null) return json({ error: '없는 나라' }, 400, h);
        const now = Date.now();
